...
const PaymentMethod = () => {
  ...
  const getWhatsAppLink = () => {
    ...
    const deliveryDetails = (pickUp) ? `Entrega en:%20${order.pickupPoint}%0A` : `Dirección:%20${order.address}%20-%20${order.district}`

    const productSummary = cart
      .map((product) => `- ${product.name} (x${product.quantity} - ${product.size} - ${product.color}): S/${(product.price * product.quantity).toFixed(2)}`)
      .join("%0A"); 

    const message = `Hola!%20Adjunto%20el%20comprobante%20de%20mi%20compra.%0A%0A` +
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      <section className="mb-8">
        ...
        <div className="bg-gray-100 p-4 rounded-lg shadow-md">
          ...
          {
            (pickUp) ? 
            (<p><strong>Recojo en:</strong> {order.pickupPoint}</p>) :
        ...
        <div className="bg-gray-100 p-4 rounded-lg shadow-md">
          {cart.map((product, index) => (
            <div key={index} className='mb-2'>
              ...
              <p className='text-xs'>Talla: {product.size} / Color: {product.color}</p>
            </div>
          ))}
          <hr className="my-4" />
      ...
      <section className="mb-8">
        <h2 className="text-2xl font-semibold mb-4">Selecciona tu Método de Pago</h2>
        <div className="flex gap-4">
          <div
            onClick={() => handlePaymentChange("Yape")}
            className={`border rounded-lg p-4 w-1/2 text-center cursor-pointer shadow-md ${
              selectedPayment === "Yape" ? "border-blue-500" : ""
            }`}
          >
            <Image src="/yape-qr.jpg" alt="Yape QR" width={150} height={150} className="mx-auto" />
          ...
          <div
            ...
            className={`border rounded-lg p-4 w-1/2 text-center cursor-pointer shadow-md ${
              selectedPayment === "Plin" ? "border-blue-500" : ""
            }`}
          >
            <Image src="/plin-qr.jpg" alt="Plin QR" width={150} height={150} className="mx-auto" />
            <p className="mt-2 font-semibold">Plin</p>
          </div>
        </div>
        <p className='text-center py-2 font-semibold'>A nombre de: Joseph Omar Meza Torres</p>
      </section>
